...
- 有追蹤過的卡片會顯示「已追蹤 N 次 · 最新 2026-09-26 21:05(今天)」,預設收合,點它才展開
- 展開後每一行是「日期 · 時間 · 操作人 · 取消」,按錯可以取消任何一筆;日期可以直接改(事後補登用,不能改成未來),追蹤、取消、改日期都會寫進修改紀錄
- **超過 7 天沒追蹤**:標籤變成黃色的「👀 9 天沒追蹤」,上方警示列顯示件數,左側分類也會亮紅點(天數在 `index.html` 的 `STALE_FOLLOW_DAYS`)
- 追蹤過的卡片會有「👀 追蹤中」標籤(滑鼠移上去顯示次數與最新時間),左側也有「👀 追蹤中」分類
- 狀態變成**暫停開發**或**已上線**後就不算追蹤中(標籤、分類都不列,追蹤紀錄仍保留)
